...
function output(options: { youtubeRate?: number | null; youtubeLabel?: string; youtubeQuality?: DataQuality; top?: boolean; noteNet?: number | null; noteQuality?: DataQuality; rakutenCommission?: number | null; rakutenQuality?: DataQuality } = {}): DashboardOutput {
  ...
  const dashboard = [
    ...
    dashboardRow("Threads", "最終データ更新", null, "2026-08-28"), dashboardRow("Threads", "直近7日間のアカウント閲覧数", 50, "50"), dashboardRow("Threads", "直前7日間のアカウント閲覧数", 50, "50"), dashboardRow("Threads", "前期間比", 0, "0%"),
...
describe("report content", () => {
  ...
  it("does not assert a Top post when ranking coverage is partial", () => {
    const value = output(); value.topContent[1] = { ...value.topContent[1]!, quality: "PARTIAL" };
    const report = text(value);
    expect(report).toContain("投稿別ランキング：集計期間不足");
    expect(report).not.toContain("最も閲覧された投稿：");
  });
