...
describe("dashboard period and quality rules", () => {
  ...
  it("handles zero comparison without Infinity", () => {
    ...
    expect(change(metric(501, "PARTIAL"), metric(21, "PARTIAL"))).toMatchObject({ value: null, label: "比較不能", quality: "PARTIAL" });
  });
...
describe("collection, activity and comparison status axes",()=>{
  ...
  it("keeps a Threads period metric inside the rolling boundary without summing snapshots",()=>{const data=empty();data.daily.push(daily("2026-08-31","threads",{views:159}));data.content.push(content("2026-08-26","threads","t1",{views:0}),content("2026-08-31","threads","t1",{views:159,publishedAt:"2026-08-31T01:00:00Z"}));const output=generateDashboard(data,"2026-09-02",collectedAt);expect(output.dashboard.find(item=>item.section==="Threads"&&item.metric==="直近7日間のアカウント閲覧数")?.value).toBe(159);expect(output.topContent.find(item=>item.platform==="threads")?.views).toBe(159);});
});
