...
const WEEKLY_HEADER = ["key", "weekStart", "weekEnd", "platform", "views", "likes", "comments", "replies", "reposts", "shares", "clicks", "orders", "grossSales", "commission", "postsPublished", "previousPeriodValue", "changeRate", "changeLabel", "overallQuality", "quality", "generatedAt"];
...
export class GoogleSheetsDashboardStore implements DashboardDataSource, DashboardSink {
  ...
  async readRawData(): Promise<RawDashboardData> {
    ...
    const importActivity = objects(logRows).filter((row) => row.source && row.finishedAt).map((row) => ({ platform: String(row.platform), source: String(row.source), status: String(row.status), finishedAt: String(row.finishedAt) }));
    return { daily, content, sales, transactions, payments, importActivity };
  }
