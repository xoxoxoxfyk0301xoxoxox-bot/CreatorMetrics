...
function platformSituation(output:DashboardOutput,section:"YouTube"|"Threads"):string{const weekly=currentWeekly(output,section.toLowerCase()),views=row(output,section,section==="YouTube"?"直近7日間の再生数":"直近7日間のアカウント閲覧数")?.value,posts=weekly?.postsPublished;if(!weekly||weekly.collectionStatus==="NO_DATA")return`${section}は対象期間の収集記録がありません。`;if(weekly.collectionStatus==="FAILED")return`${section}は対象期間のデータ取得に失敗しています。`;if(weekly.collectionStatus==="PARTIAL")return`${section}は対象期間の収集が一部のみ成功しています。`;if(weekly.collectionStatus==="STALE")return`${section}は最新データの更新待ちです。`;if(weekly.activityStatus==="ZERO_ACTIVITY")return`${section}は正常に取得できていますが、対象期間内の新規投稿・${section==="YouTube"?"再生":"アカウント閲覧"}実績はありません。`;if(weekly.activityStatus==="HAS_DATA")return`${section}は正常に取得できており、対象期間内に${(posts??0).toLocaleString("ja-JP")}投稿、${(views??0).toLocaleString("ja-JP")}${section==="YouTube"?"再生":"アカウント閲覧"}を確認しています。`;return`${section}の収集は正常ですが、対象期間の実績データがありません。`;}
function platformLines(output: DashboardOutput, section: "YouTube" | "Threads"): ReportLine[] {
  const youtube = section === "YouTube", current = row(output, section, youtube ? "直近7日間の再生数" : "直近7日間のアカウント閲覧数"), previous = row(output, section, youtube ? "直前7日間の再生数" : "直前7日間のアカウント閲覧数"), comparison = row(output, section, "前期間比");
  const weekly = currentWeekly(output, section.toLowerCase()), candidate = top(output, section.toLowerCase() as "youtube" | "threads"), best = candidate && candidate.views > 0 && candidate.quality === "OK" ? candidate : undefined, assessment = platformAssessment(output, section);
  const coverage = current?.quality === "PARTIAL" ? "（一部取得）" : current?.quality === "STALE" ? "（更新待ち）" : "";
  const lines: ReportLine[] = [{ kind: "heading", text: section }, { kind: "body", text: `直近7日間の${youtube ? "再生数" : "アカウント閲覧数"}：${display(current, "回")}${coverage}` }, { kind: "body", text: `前の7日間：${display(previous, "回")}` }, { kind: "body", text: `前期間比：${comparison?.display ?? "比較不能"}` }, { kind: "spacer", text: "" }, { kind: "body", text: `直近7日間の投稿数：${weekly?.postsPublished === null || weekly?.postsPublished === undefined ? "未取得" : `${weekly.postsPublished.toLocaleString("ja-JP")}本`}` }, { kind: "spacer", text: "" }];
  ...
  else lines.push({ kind: "body", text: candidate?.quality === "PARTIAL" || candidate?.quality === "INSUFFICIENT_BASELINE" ? `${youtube ? "動画" : "投稿"}別ランキング：集計期間不足` : youtube ? "ランキングを作成できる有効な再生データがまだありません" : "ランキングを作成できる有効な投稿別閲覧データがまだありません" });
