...
export class GoogleSheetsReportStore implements ReportSource, ReportSink {
  ...
  async readDashboardOutput(): Promise<DashboardOutput> {
    const response = await this.sheets.spreadsheets.values.batchGet({ spreadsheetId: this.spreadsheetId, ranges: ["'Dashboard'!A4:G", "'WeeklySummary'!A:X", "'TopContent'!A:R"] });
